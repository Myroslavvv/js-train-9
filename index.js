...
function findIndexByProperty(arr, name) {
  ...
  // Повертаємо знайдений індекс або -1, якщо об'єкт не знайдено.
	...
	let found = (elem => elem.name == name);
	return(arr.findIndex(found));
}
...
function findLastEvenNumber(arr) {
  ...
  let lastEvenNumber = (elem => elem %2 == 0);
  return(arr.findLast(lastEvenNumber));
}
...
console.log(
  getSubArrayAndConvertToString(
    ["Я", "люблю", "JavaScript!", "Він", "дуже", "потужний"], 3)
);
...
function getPositiveArrayValues(arr) {
  ...
  let res = Array.from(iterator);
  let even = res.filter(item => item > 0);
  return even;
}
...
function removeAndDouble(arr, startIdx) {
  ...
  if(!(Array.isArray(arr))) return "Вхідний параметр має бути масивом";
  let removed = arr.splice(startIdx);
  let doubles = removed.map((num) => num * 2);
  return doubles;

  /* let doubles = arr.map((num) => num * 2);
  return doubles; // Виведе [ 2, 4 ] */
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let a = arr.flat(2);
  let b = a.map(x => x ** 2);
  let c = b.filter(item => item > threshold);
  return(c);
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if(!(Array.isArray(arr))) return "Вхідний параметр має бути масивом";
  let removed = arr.slice(n);
    let sum = removed.reduceRight((a, b) => a + b);
	  let result = removed.forEach((elem) => {
	console.log(elem);
	});
return(sum);
}
...
function wordLengths(arr) {
  ...
  if(!(Array.isArray(arr))) return "Вхідний параметр має бути масивом";
	let lengthsArray = arr.map(function(elem) {
		return(elem.length)
	});
	return(lengthsArray);
}
...
function expandArrayByN(arr, n) {
  ...
  if(!(Array.isArray(arr)) || (isNaN(n))) return "Перший вхідний параметр має бути масивом, другий - числом";
...
function findLongestWord(arr) {
  ...
  let res = arr.reduce((long, small) => {
	if(small.length > long.length) {
		return small
	} else {
		return long;
	}
...
function findDuplicateElements(arr) {
  ...
  // Повертаємо новий масив з дубльованими елементами
	...
	if(!(Array.isArray(arr))) return [];
	let double = arr.filter((elem, index, value) => {
...
function capitalizeWords(arr) {
  ...
  // Повертаємо новий масив зі словами, у яких перша літера у верхньому регістрі
	...
	if(!(Array.isArray(arr))) return [];
	let upp = arr.map((num) => {
		let newUpp = num[0].toUpperCase() + num.slice(1);
		return newUpp;
	});
	return(upp);
}
...
function calculateTotalPrice(arr) {
  ...
  let totalPrice = arr.reduce(function (sum, elem) {
